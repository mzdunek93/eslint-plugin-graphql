...
function parseOptions(optionGroup, context) {
  ...
  if (schemaJson) {
    ...
    try {
      var config = (0, _graphqlConfig.loadConfigSync)({
        rootDir: _path2.default.resolve(process.cwd(), _path2.default.dirname(context.getFilename()))
      });
      ...
      if (projectConfig) {
        var key = config.filepath + "[" + projectConfig.name + "]";
